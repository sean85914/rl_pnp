...
suction_1_memory.load_memory("../training/manual_label/suction_1_memory.pkl")
suction_2_memory.load_memory("../training/manual_label/suction_2_memory.pkl")
gripper_memory.load_memory("../training/manual_label/gripper_memory.pkl")
...
for i in range(train_iter):
	...
	_mini_batch, _idxs, _is_weight = utils.sample_data(suction_1_memory, mini_batch_size); mini_batch += _mini_batch; idxs += _idxs; is_weight += list(_is_weight)
	_mini_batch, _idxs, _is_weight = utils.sample_data(suction_2_memory, mini_batch_size); mini_batch += _mini_batch; idxs += _idxs; is_weight += list(_is_weight)
	_mini_batch, _idxs, _is_weight = utils.sample_data(gripper_memory, mini_batch_size);   mini_batch += _mini_batch; idxs += _idxs; is_weight += list(_is_weight)
	for j in range(len(mini_batch)):
		...
		# Maybe grasp should use smaller reward?
		'''if mini_batch[j].pixel_idx[0] < 2:
			reward = mini_batch[j].reward
		else: 
			if mini_batch[j].reward == 5.0:
				reward = mini_batch[j].reward/2'''
		reward = mini_batch[j].reward
		td_target = trainer.get_label_value(reward, next_color, next_depth, mini_batch[j].is_empty)
	...
	for j in range(len(mini_batch)):
		...
		next_color = cv2.imread(mini_batch[j].next_color)
		next_depth = np.load(mini_batch[j].next_depth)
		'''if mini_batch[j].pixel_idx < 2:
			reward = mini_batch[j].reward
		else: reward = mini_batch[j].reward/2'''
		reward = mini_batch[j].reward
		td_target = trainer.get_label_value(reward, next_color, next_depth, mini_batch[j].is_empty)
		action_str, rotate_idx = utils.get_action_info(pixel_index)
	...
	if (i+1)%copy_target_net==0:
		print "Replace target net to behavior net"
		trainer.target_net.load_state_dict(trainer.behavior_net.state_dict())
