...
''' 
  Use https://github.com/andyzeng/arc-robot-vision/blob/master/parallel-jaw-grasping/convnet/processLabels.m
  to generate heightmap and augmented training label
'''
...
class parallel_jaw_based_grasping_dataset(Dataset):
    ...
    def __init__(self, n_classes, data_dir, scale=1./8):
        ...
        self.scale = scale
        f = open(self.data_dir+"train-processed-split.txt", "r")
    ...
    def __getitem__(self, idx):
        ...
        color_img = cv2.imread(self.data_dir+"color/"+idx_name+".png")
        depth_img = cv2.imread(self.data_dir+"depth/"+idx_name+".png", -1)
        label_img = cv2.imread(self.data_dir+"label-aug/"+idx_name+".png", cv2.IMREAD_GRAYSCALE)
        # uint8 -> float
...
dataset = parallel_jaw_based_grasping_dataset(3, "training/")
...
for epoch in range(epochs):
    ...
    for i_batch, sampled_batched in enumerate(dataloader):
        ...
        loss = criterion(predict.view(len(sampled_batched['color']), 3,40*40), label.view(len(sampled_batched['color']), 40*40))
        loss.backward()
...
np.savetxt("gripper_net_loss.csv", loss_l, delimiter=",")
