...
OPEN_CAM="roslaunch realsense2_camera rs_rgbd.launch camera:=camera1"
OPEN_RVIZ="rosrun rviz rviz -d src/visualization/config/system.rviz"
OPEN_SYS="roslaunch grasp_suck ict_demo.launch"
TRAIN="python main.py --epsilon 0.3 --num_of_items 50 --grasp_only --model ict_demo.pth --episode 5"
...
gnome-terminal --geometry=100x50 --working-directory=$DIR \
                                 --tab -e "bash -c \"sleep 5s; $SET_VARIABLES; roscd grasp_suck/src; $TRAIN; exec bash\""
