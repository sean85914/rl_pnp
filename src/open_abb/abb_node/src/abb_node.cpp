...
void RobotController::advertiseTopics()
{
  ...
  handle_JointsLog = 
    node->advertise<sensor_msgs::JointState>("joint_states", 100);
  handle_ForceLog = 
...
void RobotController::logCallback(const ros::TimerEvent&)
{
  ...
  msgJoints.header.frame_id = "base_link";
  msgJoints.header.stamp = ros::Time::now();
  ...
  if ((t=recv(robotLoggerSocket, buffer, MAX_BUFFER-1, 0)) > 0)
  {
    ...
    if(cartesianModif)
    {
      ...
      pthread_mutex_lock(&wobjUpdateMutex);
      handle_tf.sendTransform(tf::StampedTransform(curWobjTransform, ros::Time::now(), "base_link", "abb_workobject"));
      pthread_mutex_unlock(&wobjUpdateMutex); 
